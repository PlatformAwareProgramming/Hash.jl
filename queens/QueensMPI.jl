...
@computation cluster QueensMPI begin
    ...
    @unit master begin
        ...
        function queens(size, cutoff_depth, subproblems) 
            ...
            for i in 1:length(topology[:worker])
                ...
                MPI.send(false, MPI.COMM_WORLD; dest = topology[:worker][i], tag = 9)
                args = (size, cutoff_depth, local_subproblems)
    ...
    @unit parallel count=N worker begin
        ...
        while (!termination_flag[])
            ...
            res = QueensManycore.queens(size, cutoff_depth, subproblems)
